...
export const Projects = forwardRef(({ setSection }: { setSection: Dispatch<SetStateAction<Section>> }, ref: React.ForwardedRef<HTMLElement>) => {
    ...
    return (
        <section ref={ref} className={`h-fit w-full flex flex-col items-center bg-myBlue scroll-mt-16`}>
            <h3 className={`z-20 text-3xl font-extrabold flex justify-center items-center mb-10`}>Projects</h3>
            <div className={`z-20 w-full p-8 flex flex-wrap items-center justify-center md:bg-myBlue md:rounded-lg md:shadow-lg md:border-2 md:border-white md:w-4/6 md:bg-opacity-50 md:backdrop-filter md:backdrop-blur-sm`}  ref={projectsRef}>
